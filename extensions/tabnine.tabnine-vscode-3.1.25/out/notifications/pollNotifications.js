...
async function handleNotification({ id, message, notification_type, options }, context) {
    try {
        await utils_1.assertFirstTimeReceived(id, context);
        void setState_1.default({
            [consts_1.StatePayload.NOTIFICATION_SHOWN]: { text: message, notification_type },
        });
        return vscode.window
            .showInformationMessage(message, ...options.map((option) => option.key))
            .then((selected) => {
            const selectedAction = options.find(({ key }) => key === selected);
            void notifications_1.sendNotificationAction(id, message, selected, notification_type, selectedAction === null || selectedAction === void 0 ? void 0 : selectedAction.actions);
